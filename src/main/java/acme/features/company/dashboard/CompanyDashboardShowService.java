...
public class CompanyDashboardShowService extends AbstractService<Company, CompanyDashboard> {
	...
	final Double sessionAverage = sessionNumber != 0
		? practicumSessions.stream().mapToDouble(session -> session.getDurationInHours()).sum() / sessionNumber
		: 0.;
	final Double sessionDeviation = sessionNumber != 0 ? Math.sqrt(practicumSessions.stream()
		.mapToDouble(session -> Math.pow(session.getDurationInHours() - sessionAverage, 2)).sum()
		/ sessionNumber) : 0.;
	final Double sessionMax = practicumSessions.stream().mapToDouble(session -> session.getDurationInHours()).max()
		.orElse(0.0);
	final Double sessionMin = practicumSessions.stream().mapToDouble(session -> session.getDurationInHours()).min()
		.orElse(0.0);
