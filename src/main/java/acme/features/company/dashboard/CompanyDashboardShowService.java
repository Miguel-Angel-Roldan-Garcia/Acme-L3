...
public class CompanyDashboardShowService extends AbstractService<Company, CompanyDashboard> {
				...
				if (currentYear == psInitialYear && currentYear == psEndYear) { // Inicio y fin en el año actual
					if (currentMonth == psInitialMonth && currentMonth == psEndMonth) { // Inicio y fin en el mes actual
						if (currentDay >= psInitialDay && currentDay >= psEndDay) // Inicio y fin previo al dia actual
							counted.set(11, true);
					} else if (currentMonth - 1 == psInitialMonth && currentMonth == psEndMonth) { // Inicio en el mes anterior y fin en el mes actual
						if (currentDay >= psEndDay) { // Día final previo al día actual
							counted.set(11, true);
							counted.set(10, true);
						}
					} else if (currentMonth > psEndMonth && psEndMonth != psInitialMonth) { // Inicio y fin en meses distintos
						counted.set(psInitialMonth + 11 - currentMonth, true);
						counted.set(psEndMonth + 11 - currentMonth, true);
					} else if (currentMonth > psEndMonth && psInitialMonth == psEndMonth) // Inicio y fin en meses iguales previos
						counted.set(psEndMonth + 11 - currentMonth, true);

				} else if (currentYear - 1 == psInitialYear && currentYear == psEndYear) { // Inicio en el año anterior y fin en el actual
					counted.set(11 - currentMonth, true);
					counted.set(11 - currentMonth - 1, true);

				} else if (currentYear - 1 == psInitialYear && currentYear - 1 == psEndYear) // Inicio y fin en el año anterior
					if (currentMonth < psInitialMonth && psEndMonth != psInitialMonth) { // Inicio y fin en meses distintos y comprueba que la sesión este en los últimos 12 meses
						counted.set(psInitialMonth - currentMonth - 1, true);
						counted.set(psEndMonth - currentMonth - 1, true);
					} else if (currentMonth < psInitialMonth && psInitialMonth == psEndMonth) // Inicio y fin en meses iguales previos y comprueba que la sesión este en los últimos 12 meses
						counted.set(psEndMonth - currentMonth - 1, true);
					else if (currentMonth < psEndMonth && psInitialMonth != psEndMonth) // Inicio y fin en meses distintos para el caso del último mes y el anterior
						counted.set(psEndMonth - currentMonth - 1, true);
			}
	...
	public void unbind(final CompanyDashboard object) {
		...
		tuple.put("displayLenguage", currentLocale.getDisplayLanguage());
