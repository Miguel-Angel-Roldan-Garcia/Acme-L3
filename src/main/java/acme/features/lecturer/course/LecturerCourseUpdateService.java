...
public class LecturerCourseUpdateService extends AbstractService<Lecturer, Course> {
	...
	if (!super.getBuffer().getErrors().hasErrors("retailPrice")) {
	    ...
	    super.state(moneyAmountStatus, "retailPrice", "lecturer.course.error.negative");
	}
	...
	if (!super.getBuffer().getErrors().hasErrors("retailPrice")) {
	    ...
	    super.state(moneyCurrencyStatus, "retailPrice",
		    "administrator.offer.form.error.price.non-existent-currency");
	}
	...
	for (final Lecture lecture : lectures)
		...
		theoreticalCount++;
	final Nature nature = theoreticalCount == handsOnCount ? Nature.BALANCED
		: theoreticalCount > handsOnCount ? Nature.THEORETICAL : Nature.HANDS_ON;

	final boolean publishable = object.isDraftMode() && lectures != null && !lectures.isEmpty();
