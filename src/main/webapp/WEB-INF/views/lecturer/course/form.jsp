...
<acme:form readonly="${!draftMode}"> 
	...
	<acme:input-money code="lecturer.course.form.label.retail-price" path="retailPrice"/>
		...
		<jstl:when test="${_command == 'show' && draftMode == false}">
			<acme:button code="lecturer.course.form.button.lectures-in-course" action="/lecturer/lectures/list?masterId=${id}"/>			
		</jstl:when>
		<jstl:when test="${acme:anyOf(_command, 'show|update|delete|publish') && draftMode == true}">
			<acme:button code="lecturer.course.form.button.lectures-in-course" action="/lecturer/lecture/list?courseId=${id}"/>			
